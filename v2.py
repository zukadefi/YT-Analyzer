...
if all(x > 0 for x in [implied_apy, underlying_apy, pt_price, d, yt_now]):
    ...
    if (implied_apy > underlying_apy) and (yt_now > fair_yt) and percentage_apy > 1 and percentage > 1:
        ...
        if percentage_apy < 1:
            if yt_now < fair_yt:
                st.info(f"Implied APY is nearly equal to Underlying APY, but YT price is cheap (`{percentage_apy:.2f}%` lower than its fair price based on the maturity curve).")
            elif yt_now > fair_yt:
    ...
    if calc_points:
        ...
        if total_points > 0:
            ...
            st.info(f"At maturity, you will have {user_points} points. This is {share:.6f}% of all distributed points.")
